 ...
 */
  ...
  if (!this.locationSearchEmulated)
  {
    try
    {
      ...
      var key = document.parentWindow.frameElement.id;
    ...
    if (!this.locationSearchEmulated)
    {
      ...
      if (das && das.LocationSearch) this.locationSearchEmulated = das.LocationSearch;
    }
      
    if (!this.locationSearchEmulated) this.locationSearchEmulated = "empty";
  }
  ...
  if (!parsedData) {
    ...
    for (var i=0; i<searchArr.length; i++) {
      ...
      if (storedValues == undefined) {
        ...
        storedValues.toString = new Function("return this.join(', ');");
  ...
  if (variable == undefined) return document.location.search.substr(1);
  else {
...
QueryString(); //a call to initialize QueryString
